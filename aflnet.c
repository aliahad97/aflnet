...
region_t* extract_requests_rtps(unsigned char* buf, unsigned int buf_size, unsigned int* region_count_ref)
{
  ...
  unsigned int region_count = 0;
  region_t *regions = NULL;
  char terminator[4] = {0x52, 0x54, 0x50, 0x53};
  ...
  while (byte_count < buf_size) {
    ...
    //Check if the RTPS packet ended
    if ((mem_count > 3) && 
    // ((memcmp(&mem[mem_count - 3], terminator, 4) == 0)) && // Last four bytes are RTPS
    ((buf_size - byte_count >= 4) && (memcmp(buf + byte_count, "RTPS", 4) == 0)) // next packet started
    ) {
      region_count++;
      regions = (region_t *)ck_realloc(regions, region_count * sizeof(region_t));
      regions[region_count - 1].start_byte = cur_start;
...
unsigned int* extract_response_codes_rtps(unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref)
{
  ...
  for (unsigned int byte_count = 0; byte_count < buf_size; byte_count++) {
    ...
    if ((buf_size - byte_count >= 4) && (memcmp(buf + byte_count, "RTPS", 4) == 0)){
        byte_count += 19; // skip first 20 bytes of header to submessages
        // printf("Found header and skipping\n");
        continue;
    }
    if (buf_size - byte_count < 4) break;

    // Parse submessage
    // 1. Get submessage ID:
    unsigned int message_code = (unsigned int) (buf[byte_count]);
    // save the id as state:
    state_count++;
    state_sequence = (unsigned int *)ck_realloc(state_sequence, state_count * sizeof(unsigned int));
    state_sequence[state_count - 1] = message_code;
    byte_count ++; // Message ID
    byte_count ++; // Flag ID
    // skip bytes:
    unsigned int skip_bytes = (unsigned char)buf[byte_count + 1] << 8 | (unsigned char)buf[byte_count]; //
    byte_count += 1; // message data
    byte_count += skip_bytes; // skip payload
  }
  *state_count_ref = state_count;
  return state_sequence;
}
